...
public class NoHeadsBeforeGetsTransferManager {
    ...
    /** With range **/
    public Download download(String bucket, String key, File file, long byteSize, int offset, int length) {
        GetObjectRequest gor = new GetObjectRequest(bucket, key);
        if(offset >= 0 && length != -1)
        {
            gor.setRange(offset, offset + length);
        }
        return download(gor, file, byteSize);
    }
    ...
    private Download doDownload(final GetObjectRequest getObjectRequest,
        ...
        objectMetadata.setLastModified(new Date()); //TODO: hacky stuff
        ...
        final DownloadImpl download = new DownloadImpl(description,
                ...
                getObjectRequest, file, objectMetadata, false);
