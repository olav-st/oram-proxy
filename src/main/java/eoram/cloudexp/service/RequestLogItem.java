...
{	
		...
		if(summarizeOps == true)
		{
			...
			if(operationsMap.size() > 0)
			{
				...
				for(long opId : operationsMap.keySet())
				{
					...
					OperationType opType = opli.getType();
					...
					if(opli.isComplete() == true)
					{
						...
						case COPY: numC++; break;
						case LIST: numL++; break;
						}
	...
	public int getOpsCount(OperationType type) 
	{
		int ret = 0;
	...
	public double getOpsKB(OperationType type) 
	{
		double ret = 0.0;
	...
	public List<Double> getOpsTimes(OperationType type)
	{
