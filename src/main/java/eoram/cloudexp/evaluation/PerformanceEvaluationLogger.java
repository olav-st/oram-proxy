...
public class PerformanceEvaluationLogger
{
	...
	protected Map<Long, RequestLogItem> requestsMap = new ConcurrentHashMap<Long, RequestLogItem>();
	...
	public void scheduleRequest(Request req) 
	{
		...
		if(req.getType() == RequestType.PUT)
		{
			...
			long byteSize = put.getValue().getNumBytes();
	...
	public void completeRequest(ScheduledRequest sreq) 
	{
		...
		{
			long byteSize = 0;
			if(sreq.wasSuccessful() == true) { byteSize = sreq.getDataItem().getNumBytes(); }
			logItem.setItemByteSize(byteSize);
		}
		logItem.completed();
	}
	...
	public void scheduleOperation(Operation op)
	{
		...
		if(op.getType() == OperationType.UPLOAD)
		{
			...
			long byteSize = up.getDataItem().getNumBytes();
	...
	public void completeOperation(ScheduledOperation sop)
	{
		...
		if(op.getType() == OperationType.DOWNLOAD || op.getType() == OperationType.DELETE)
		{
			...
			{ if(sop.wasSuccessful() == true) {  byteSize = sop.getDataItem().getNumBytes(); } }
			opLogItem.setObjectByteSize(byteSize);
		}
		...
		try
		{
			...
			for(long reqId : new TreeSet<Long>(requestsMap.keySet()))
			{
				...
				if(reqLogItem.isInit() == false) 
				{
					...
					double dOps = reqLogItem.getOpsCount(OperationType.DOWNLOAD); dOpsCountList.add(dOps);
			...
			if(totalReqCount > 0)
			{
				...
				bw.write("Total download operations (GET): " + downloadOps);
				bw.newLine();
				bw.write("Total upload operations (PUT): " + uploadOps);
