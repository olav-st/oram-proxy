...
import eoram.cloudexp.construction.SchemeFactory;
...
import eoram.cloudexp.service.application.*;
import eoram.cloudexp.service.runners.*;
import eoram.cloudexp.test.ClientTester;
...
/**
 * Main entry point. This class contains the logic to run requests on top 
 * of an ORAM client (specified by name) interacting with a storage backend (specified by name).
 */
public class CloudExperiments 
{
	...
	private static final String proxyArg = "http-proxy";
	private static final int defProxyArg = 12345;
	...
	public static void main(String[] args) throws Exception {
		...
		if(!sessionFile.getParentFile().exists() || !sessionFile.exists())
		{
			sessionFile.getParentFile().mkdirs();
			try { sessionFile.createNewFile(); } catch (IOException e) { e.printStackTrace(); }
		}
		...
		{			
			...
			proxyRS = containsOptionalFlag(args, proxyArg);
			if(proxyRS)
			{
				ss.httpProxyPort = getArgIntValue(args, proxyArg, defProxyArg);
			}
		...
		final String expDescStr = "Experiment desc:\n" + experimentDesc;
		...
		if(rs == null) 
		{
			if(proxyRS == true)
			{
				short proxyPort = (short) getArgIntValue(args, proxyArg, defProxyArg);
				log.append("Waiting for requests over HTTP on port " + proxyPort, Log.INFO);
				rs = new HTTPServerRequestSource(proxyPort);
			}
			else if(ss.extracted == true)
			{
	...
	private static long defMaxBlocks = 1 << 12;
	...
	private static int defLocalPosMapCutoff = 256 * 1024 * 1024;
	...
	private static String getExperimentDesc()
	{
		...
		String inputDesc;
		if(ss.httpProxyPort != null)
		{
			inputDesc = "[Input] HTTP proxy on port " + ss.httpProxyPort + " (test: " + ss.testOnly + ")";
		}else
		{
			inputDesc = "[Input] rsFile: " + ss.rsFilePath + " (test: " + ss.testOnly + ")";
		}
