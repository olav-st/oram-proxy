...
public class Stash {
	...
	private int[] counter;
	private int stashSize, peakStashSize;
	
	public Stash(){
		this.stashSize = 0;
		this.peakStashSize = 0;
	...
	public void add(Block blk){
		if(!stash_hash.containsKey(blk.getBlockIndex())){
			...
			stashSize += 1;
			if(stashSize > peakStashSize)
			{
				peakStashSize = stashSize;
			}
		}
	}
	...
	public int getPeakStashSize() {
		return peakStashSize;
	}
